        ...
        _this.validate = function (form) {
            ...
            if (validator && form.model) {
                if (validator.validateSync) {
                    try {
                        validator.validateSync(form.model, __assign({ abortEarly: false, context: {
                                state: _this.state,
                                props: _this.props,
                        ...
                        if (_errors.path === undefined) {
                            _errors.inner.forEach(function (error) {
                                errors = __assign({}, errors, (_a = {}, _a[error.path] = (errors[error.path] || []).concat(error.errors), _a));
                                var _a;
                            });
                        }
                        else {
            ...
            if (scopeValidator && form.model) {
                var preScope = scopeValidator.validate(form.model, {
                    state: _this.state,
                    props: _this.props,
                });
