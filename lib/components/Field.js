...
var FieldClass = (function (_super) {
    ...
    FieldClass.getDerivedStateFromProps = function (_a, _b) {
        ...
        if (!tools_1.isArrayEqual((validationScope || []).map(function (error) { return error.message; }), (nextValidationScope || []).map(function (error) { return error.message; }))) {
    ...
    FieldClass.prototype.shouldComponentUpdate = function (nextProps, nextState) {
        ...
        if (onChange !== nextProps.onChange
            ...
            || !tools_1.isArrayEqual((validationErrors || []).map(function (error) { return error.message; }), (nextState.validationErrors || []).map(function (error) { return error.message; }))
            || !tools_1.isArrayEqual((validationScope || []).map(function (error) { return error.message; }), (nextState.validationScope || []).map(function (error) { return error.message; }))
