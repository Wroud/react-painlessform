...
class FieldClass extends React.Component {
    ...
    render() {
        ...
        if (form.storage.config.htmlPrimitives) {
            const value = form_1.getDefaultValue(fieldValue || defaultValue, type, multiple);
            inputHook = {
                name: name.getPath(),
                type,
                value: form_1.getInputValue(value, type, forwardedValue, multiple),
                checked: form_1.getInputChecked(value, type, forwardedValue),
                multiple,
    ...
    mountValue() {
        ...
        if (value === Unset) {
            this.update(form_1.getDefaultValue(defaultValue || Unset, type, multiple), {
                isVisited: false,
                isChanged: false,
                isFocus: false
            });
